...
def main():
    ...
    # Modelo: OpenCode Zen (gateway compatible OpenAI) — modelo gratis "big-pickle"
    # El endpoint y la API key se inyectan via OPENAI_API_KEY en docker-compose.
    # Cuando cerebropax (LiteLLM) esté desplegado, se migrará a ese endpoint.
    ...
    agent_payload = {
        ...
        "model_endpoint": OPENCODE_ZEN_ENDPOINT,
    ...
    if resp.status_code == 200 or resp.status_code == 201:
        ...
        print(f"   ID: {agent_id}")
        print(f"   Modelo: {OPENCODE_ZEN_MODEL} (OpenCode Zen, gratis)")
